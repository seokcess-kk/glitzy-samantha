...
| `lead-custom-data.ts` | `extractCustomEntries`, `formatCustomValue` | 리드 `custom_data`(JSONB) 표시용 평탄화 — 사용자 **선택값만** 노출(selected_option_name·price 우선, price 통화 포맷). 전체 메뉴/템플릿 설정값(`options`·`product`·`price_note`·`event_period`)·객체 배열 카탈로그·내부 키(name·marketing_consent 등) 제외, survey 중첩 펼침, `[object Object]` 방지. leads·campaigns 페이지 공용 |
