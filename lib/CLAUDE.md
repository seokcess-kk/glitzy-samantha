...
| `lead-custom-data.ts` | `extractCustomEntries`, `formatCustomValue` | 리드 `custom_data`(JSONB) 표시용 평탄화 — survey 중첩/평면/불리언/배열 안전 처리, 내부 키(name·marketing_consent 등) 제외, `[object Object]` 방지. leads·campaigns 페이지 공용 |
