...
### 선택 (서비스별)
- 광고: `GOOGLE_ADS_CLIENT_ID`/`GOOGLE_ADS_CLIENT_SECRET`/`GOOGLE_ADS_DEVELOPER_TOKEN`/`GOOGLE_ADS_REFRESH_TOKEN`/`GOOGLE_ADS_LOGIN_CUSTOMER_ID`/`GOOGLE_ADS_CUSTOMER_ID`(MCC 공통 자격증명 ENV fallback, `clinic_api_configs.google_ads`에 `customer_id`만 입력해도 동작), `META_*`, `TIKTOK_ADVERTISER_ID`, `TIKTOK_ACCESS_TOKEN` (+ 병원별 `clinic_api_configs`)
...
| `clinic_api_configs` | 병원별 광고 API 키 + 부가 연동 | `platform` = 광고 매체 6종 + `meta_capi`(전환 API: `pixel_id`·`access_token`). `is_active`로 활성/비활성. 광고 동기화는 `SYNC_ENABLED_PLATFORMS`만 대상(meta_capi 제외) |
| `clinic_ad_markup` | 병원별 광고비 마크업(관리 수수료) | 읽기 시점 가산 설정. campaign_id·daily_amount·effective_from/to. DB 원본 불변, `lib/ad-markup.ts` 참조 |
...
| 엔드포인트 | 용도 |
|-----------|------|
| `GET /api/external/ad-spend` | 병원별 월간 광고 **실집행비** + SMS 건수 (광고비 마크업 미적용 — 정산 기준) |
...
| `utm.ts` | `parseUtmFromUrl`, `sanitizeUtmParams`, `mergeUtmParams`, `parseLandingPageIdFromUrl` | UTM 파싱/검증 + inflow_url 의 `id=` → landing_page_id 추출(webhook 귀속 복구용) |
| `lead-custom-data.ts` | `extractCustomEntries`, `formatCustomValue` | 리드 `custom_data`(JSONB) 표시용 평탄화 — 사용자 **선택값만** 노출(selected_option_name·price 우선, price 통화 포맷). 전체 메뉴/템플릿 설정값(`options`·`product`·`price_note`·`event_period`)·객체 배열 카탈로그·내부 키(name·marketing_consent 등)·광고 클릭 ID(`fbclid`·`gclid`·`ttclid` 등)·UTM 키(`utm_*`)·페이지 언어(`lang`) 제외, survey 중첩 펼침, `[object Object]` 방지, ISO 타임스탬프 값(예: `agreed_at`)은 KST `YYYY-MM-DD HH:mm` 자동 변환. leads·campaigns 페이지 공용 |
...
| `archive.ts` | `archiveBeforeDelete`, `archiveManyBeforeDelete`(ID 배열 벌크), `archiveBulkBeforeDelete`(filterColumn 벌크) | 삭제 전 스냅샷 보관 |
| `platform.ts` | `API_PLATFORMS`, `ConfigPlatform`, `API_CONFIG_PLATFORMS`, `CONNECTION_TEST_PLATFORMS`, `CAMPAIGN_TYPES_BY_PLATFORM`, `isApiPlatform`/`isConfigPlatform` 등 | 광고 플랫폼 & 캠페인 타입 중앙 상수 (Single Source of Truth). `ApiPlatform`=광고 매체 6종(동기화/캠페인/소재 대상), `ConfigPlatform`=광고+`meta_capi`(설정 UI 전용, 광고 도메인 미포함) |
...
| `date.ts` | `formatDate`, `formatDateTimeYmdHm`, `toUtcDate`, `getKstDateString`, `getKstDayStartISO`, `getKstDayEndISO` | KST 기준 날짜 포맷/생성 (아래 타임존 규칙 참조) |
| `ad-markup.ts` | `fetchAdMarkups`, `buildMarkupStatRows`, `markupTotal`, `markupCampaignIds`, `MARKUP_HINT` | 광고비 마크업(관리 수수료) 읽기 시점 가산. DB 원본(ad_campaign_stats) 불변, 조회 시 합성 행/스칼라로 spend 가산 → 파생 지표 자동 반영. 고지 동반 필수 |
...
| `services/metaCapi.ts` | Meta CAPI 전송 | 리드 유입 시 서버사이드 전환 이벤트 전송. 설정 우선순위: `clinic_api_configs`(`meta_capi`, `is_active`+복호화 존중) → `META_PIXEL_ID`/`META_ACCESS_TOKEN` ENV 폴백 |
...
| `formatDateTimeYmdHm(str\|Date)` | `"2026-03-30 14:30"` | 연도 포함 표시용 날짜+시간 (KST, 24시간제) |
