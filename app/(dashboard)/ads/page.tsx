      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-8">
        {[
          ...
          <Card key={label} variant="glass" className="p-3">
            ...
            {loading ? <Skeleton className="h-6 mt-1" /> : <p className="text-lg font-bold text-foreground truncate">{value}</p>}
          </Card>
        ))}
      </div>
            ...
            <div className="min-w-[360px]">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={platformFilter === 'all' ? channelData : filteredChannel} barGap={6}>
                  ...
                  <XAxis dataKey="channel" tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} axisLine={false} tickLine={false} />
                  <YAxis yAxisId="left" tickFormatter={(v: number) => `₩${(v / 1000).toFixed(0)}k`} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis yAxisId="right" orientation="right" tickFormatter={(v: number) => `${(v * 100).toFixed(0)}%`} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 11 }} axisLine={false} tickLine={false} />
                  <Tooltip
      ...
      <Card variant="glass" className="p-6">
        <div className="flex items-center justify-between mb-5 gap-4">
          <h2 className="font-semibold text-foreground shrink-0">
            캠페인별 상세 지출 내역
            {platformFilter !== 'all' && <span className="ml-2 text-sm text-muted-foreground">({platformFilter})</span>}
          </h2>
          <div className="flex items-center gap-2">
            <Input
              ...
              className="w-48"
            />
            <span className="text-xs text-muted-foreground shrink-0">{filteredStats.length}건</span>
          </div>
        </div>
        {loading ? (
              ...
              <TableBody>
                {filteredStats.map((row: any) => (
                  <TableRow key={row.id} className="border-b border-border dark:border-white/5 hover:bg-muted dark:hover:bg-white/[0.03]">
                    ...
                    <TableCell className="text-foreground/80 max-w-[200px] truncate">{row.campaign_name || row.campaign_id}</TableCell>
